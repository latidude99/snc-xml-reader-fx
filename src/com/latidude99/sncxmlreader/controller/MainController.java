...
import com.latidude99.sncxmlreader.app.MessageBox;
import com.latidude99.sncxmlreader.app.MessageBoxOn;
...
//					exJAXB.printStackTrace();
		...
		buttonCatInfo.setOnAction(new EventHandler<ActionEvent>(){
			...
			public void handle(ActionEvent event) {
				if(ukhoCatalogueFile != null) {
					...
					MessageBox.show("The UKHO Standard Navigation Chart catalogue has not been loaded yet.", "Info");
				}
	...
	private void configureProcessing() {
		...
		buttonSearchChart.setOnAction(new EventHandler<ActionEvent>(){
			...
			public void handle(ActionEvent event) {
				if(ukhoCatalogueFile == null) {
					MessageBox.show("The UKHO Standard Navigation Chart catalogue has not been loaded yet.\n"
		...
		linkAbout.setOnAction(new EventHandler<ActionEvent>() {
			...
			public void handle(ActionEvent event) {
				...
				sb.append("Standard Navigation Chart XML Reader (SncXmlReader) v.0.1 beta\r\n");
				sb.append("--------------------------------------------------------------------------\r\n\n");
				sb.append("The goal of SncXmlReader is to present a UKHO Standard Navigation Chart catalogue\r\n"
		...
		linkHelp.setOnAction(new EventHandler<ActionEvent>() {
			...
			public void handle(ActionEvent event) {
				StringBuilder sb = new StringBuilder();
				sb.append("Standard Navigation Chart XML Reader \r\n");
				sb.append("-------------------------------------------------------------------------------------------------------------------------------\r\n");
...
//		System.out.println(daysBetween + ", " + daysToNew);
	...
	public String diplaySingleChart(UKHOCatalogueFile catalogue, String chartNumber) {
		...
		if(!chartsFound.isEmpty() && chartsFound.get(0).getMetadata().getCatalogueNumber().equals(chartNumber))
			...
			String message = "Chart number " + chartNumber + " has not been found in the catalogue.";
